...
def login():
    ...
    tesadufen bulan birine burada bir panel oldugunu soylerdi. Ayni
    sebeple kilitliyken de 404 doner -- kilidin varligi da bilgi sizdirir.
    """
    ...
    if request.method == "POST":
        ...
        if security.sifre_dogru(request.form.get("parola", "")):
            ...
            security.giris_yap()
            hedef = request.args.get("next") or url_for("admin.pages")
            # acik yonlendirme olmasin: sadece kendi adresimize don
            if not hedef.startswith(current_app.config["ADMIN_PATH"]):
                hedef = url_for("admin.pages")
...
def change_password():
    ...
    return render_template(
        "admin/change_password.html", csrf=security.csrf_token())
